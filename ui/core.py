...
class Reactive:
    ...
    def track(self, event, x, y, flags, param):
        ...
        # Verifica si el clic está dentro del cuadro 'line'
        if event == cv2.EVENT_LBUTTONDOWN and self.is_hovered(x, y):
            if self.bd_callback:
                self.bd_callback()
        # Verifica si el clic está dentro del cuadro 'line'
        if event == cv2.EVENT_LBUTTONUP and self.is_hovered(x, y):
            if self.bu_callback:
                self.bu_callback()
...
class UI:
    def get_position(self):
        return (self.position[0],
                self.position[1], self.position[0] + self.style.width,
                self.position[1] + self.style.height)
